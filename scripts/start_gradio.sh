...
# ==============================================================================
export EXTERNAL_HEAD_IP=$(ray get-head-ip infra/cluster-serve.yaml)
export RAY_HEAD_IP=$(gcloud compute instances list | grep $EXTERNAL_HEAD_IP | awk '{print $4}')
unset RAY_ADDRESS

cd serve/
serve run --working-dir="." --address=ray://${RAY_HEAD_IP}:10001 -h 0.0.0.0 -p 8000 llama_serve:gradio
cd ..
